...
function removeBlockLoader(element){if($(element).attr('id')==='customer-email-fieldset'||$(element).attr('id')==='opc-shipping_method'||($(element).hasClass('opc-block-summary')&&$(element).hasClass('_block-content-loading'))){$(document).find(blockLoaderClass).hide();}
if(!element.has(blockLoaderClass).length){return;}
